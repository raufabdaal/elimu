...
export default function PricingPage() {
  ...
  const handleActivationRequest = async (plan: PaymentPlan) => {
    ...
    try {
      ...
      setMessage(`Activation request saved. Reference: ${payment.external_reference}. For now, this is a manual pilot request: confirm payment with the Elimu team, then your account will be activated in Supabase.`);
      await refresh();
    } catch (e) {
          ...
          {payments.length > 0 && (
            <section className="rounded-[28px] border-2 border-slate-200 bg-white p-4 shadow-sm">
              ...
              <p className="mt-1 text-xs font-semibold text-slate-500">
                Pending means the request was saved. During the pilot, activation is completed manually after payment confirmation.
              </p>
